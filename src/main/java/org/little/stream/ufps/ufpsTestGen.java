package org.little.stream.ufps;

import java.util.Date;
import org.little.util.Except;
import org.little.util.string.stringDate;

public class ufpsTestGen {

       public ufpsTestGen() {
       }

       public static ufpsMsg getDef(){
              return new ufpsMsg(def_msg2.getBytes());
       }
       public static ufpsMsg getCalc(){
              ufpsMsg msg=new ufpsMsg();

              msg.addTO        ("uic:"+System.currentTimeMillis());       
              msg.setFROM      ("uic:"+System.currentTimeMillis());     
              msg.setType      ("5");      
              msg.setPriority  ("1");  
              msg.setID        ("guid:"+System.currentTimeMillis());        
              msg.setAppID     ("guid:"+System.currentTimeMillis());     
              msg.setCorID     ("guid:"+System.currentTimeMillis());     
              msg.setCreateTime(stringDate.date2Z(new Date()));    
              msg.setSendTime  (stringDate.date2Z(new Date()));  
              msg.setReceveTime(stringDate.date2Z(new Date()));
              msg.setAcceptTime(stringDate.date2Z(new Date()));
              msg.setAckRequest("true");
              msg.setDocID     (""+System.currentTimeMillis());        
              msg.setDocType   ("ED701");        
              msg.setDocFormat (System.currentTimeMillis()+"-"+System.currentTimeMillis());        


              return msg;
       }

       public static final String def_msg1=""
               +"<?xml version=\"1.0\" encoding=\"WINDOWS-1251\"?>\n"
               +"<env:Envelope xmlns:env=\"http://www.w3.org/2003/05/soap-envelope\">\n"
               +"  <env:Header>\n"
               +"      <props:MessageInfo xmlns:props=\"urn:cbr-ru:msg:props:v1.3\">\n"
               +"           <props:To>uic:040700199900</props:To>\n"
               +"           <props:From>uic:049999900000</props:From>\n"
               +"           <props:MessageID>450295413-0000004099013</props:MessageID>\n"
               +"           <props:MessageType>1</props:MessageType>\n"
               +"           <props:Priority>5</props:Priority>\n"
               +"           <props:CreateTime>2015-12-16T19:50:08Z</props:CreateTime>\n"
               +"      </props:MessageInfo>\n"
               +"      <props:DocInfo xmlns:props=\"urn:cbr-ru:msg:props:v1.3\">\n"
               +"           <props:DocFormat>2</props:DocFormat>\n"
               +"           <props:DocType>MT865</props:DocType>\n"
               +"           <props:DocID>450295413-0000004099013</props:DocID>\n"
               +"      </props:DocInfo>\n"
               +"   </env:Header>\n"
               +"   <env:Body>\n"
               +"   <Object xmlns=\"urn:cbr-ru:dc:v1.0\"></Object>\n"
               +"   </env:Body>\n"
               +"</env:Envelope>\n"
               +"\n"
               ;
       public static final String def_msg2=""
               +"<env:Envelope xmlns:env=\"http://www.w3.org/2003/05/soap-envelope\">\n"
               +"  <env:Header>\n"
               +"    <props:MessageInfo xmlns:props=\"urn:cbr-ru:msg:props:v1.3\">\n"
               +"      <props:To>uic:451111111100</props:To>\n"
               +"      <props:From>uic:452222222211</props:From>\n"
               +"      <props:AppMessageID>guid:sat-00000094.22765.6577674.2202603.1.SYS.FAST.PAYMENT.xml-unicend</props:AppMessageID>\n"
               +"      <props:MessageID>guid:sat-00000094.22765.6577674.2202603.1.SYS.FAST.PAYMENT.xml-unicend</props:MessageID>\n"
               +"      <props:MessageType>1</props:MessageType>\n"
               +"      <props:Priority>5</props:Priority>\n"
               +"      <props:CreateTime>2018-08-10T05:07:01Z</props:CreateTime>\n"
               +"      <props:AckRequest>true</props:AckRequest>\n"
               +"    </props:MessageInfo>\n"
               +"    <props:DocInfo xmlns:props=\"urn:cbr-ru:msg:props:v1.3\">\n"
               +"      <props:DocFormat>1</props:DocFormat>\n"
               +"      <props:DocType>ED701</props:DocType>\n"
               +"      <props:EDRefID EDNo=\"302146116\" EDDate=\"2018-08-10\" EDAuthor=\"4522222222\"/>\n"
               +"    </props:DocInfo>\n"
               +"  </env:Header>\n"
               +"  <env:Body>\n"
               +"    <sen:SigEnvelope xmlns:sen=\"urn:cbr-ru:dsig:env:v1.1\">\n"
               +"      <sen:SigContainer>\n"
               +"        <dsig:MACValue xmlns:dsig=\"urn:cbr-ru:dsig:v1.1\"></dsig:MACValue>\n"
               +"      </sen:SigContainer>\n"
               +"      <sen:Object></sen:Object>\n"
               +"    </sen:SigEnvelope>\n"
               +"  </env:Body>\n"
               +"</env:Envelope>\n"
               +"\n"
               ;

       public static void main(String[] args) {
              try {
                  System.out.println("Start");
                  ufpsMsg msg=null;
                  msg=ufpsTestGen.getDef();
                  System.out.println("Def:"+msg.toString());

                  msg=ufpsTestGen.getCalc();
                  System.out.println("Calc:"+msg.toString());
              } catch (Exception e) {
                  e.printStackTrace();
              }
                  
       }

}





